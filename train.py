...
class Transformer:
    ...
    def generate(self, params, inputs, max_new_tokens, temperature=1.0, key=None):
        ...
        # avoiding JAX recompilation on every new token.
        buffer = jnp.zeros((batch_size, self.max_len), dtype=inputs.dtype)
        buffer = buffer.at[:, :prompt_len].set(inputs[:, -self.max_len:])
        cur_len = min(prompt_len, self.max_len)

        for _ in range(max_new_tokens):
            # Forward pass with fixed-shape buffer (no recompilation)
            logits = self.forward(params, buffer, key, training=False)
            ...
            # Write into buffer (shift left if we've hit max_len)
